...
<state>
branch: feat/m7-charlotte-audit (off main 1e01f20) | date: 2026-05-21 | tests: need to run on user machine (sandbox disk full). Pre-existing suite: 34+ tests passing per prior state. M7 adds 6 new tests in tests/test_charlotte_audit.py.
m7-charlotte-audit-2026-05-21: NEW jeeves/tools/charlotte.py: async fetch_url_via_charlotte() — spawns Charlotte MCP subprocess (npx @ticktockbent/charlotte --profile core), MCP JSON-RPC 2.0 initialize handshake → navigate → observe, returns page text. Never raises. NEW scripts/audit.py additions: (a) import asyncio at top; (b) charlotte_verified_count/charlotte_flagged_count fields in AuditReport; (c) _cerebras_verify_claim() calls Cerebras /v1/chat/completions directly via httpx with llama-3.3-70b → llama3.1-70b chain, returns YES/NO/''; (d) _extract_anchor_context() extracts ±200 chars around anchor tag; (e) verify_urls_with_charlotte() fetches up to 20 URLs via Charlotte+Cerebras, appends hallucinated_url defects on NO verdict; (f) run_audit() gains use_charlotte kwarg, wires D_charlotte_url_verify into detectors_run/skipped; (g) main() gains --force-charlotte arg + JEEVES_USE_CHARLOTTE_AUDIT env check + charlotte stats in summary output. MODIFIED jeeves/schema.py: added AuditResult dataclass + AUDIT_DEFECT_TYPES frozenset (satisfies grep check for hallucinated_url). MODIFIED .github/workflows/daily.yml: Install Charlotte MCP step (if-gated on var), CEREBRAS_API_KEY + JEEVES_USE_CHARLOTTE_AUDIT env vars plumbed to audit step. NEW tests/test_charlotte_audit.py: 6 hermetic tests (FileNotFoundError stub, timeout stub, flag-off via run_audit mock, Cerebras NO → defect, Cerebras YES → skip, 20-URL cap).
