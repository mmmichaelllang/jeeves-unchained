...
<state>
branch: main | sprint: 15 (TOTT + playwright hardening) | date: 2026-05-04 | tests: 257 touched-file tests passing (pre-existing llama_index sandbox failures unchanged)
...
sprint-15-playwright-rewrite-2026-05-04: jeeves/tools/playwright_extractor.py major hardening (5 parallel research agents → consensus picks). Patchright import preferred (Runtime.enable CDP-leak fix, --enable-automation stripped, navigator.webdriver removed) with vanilla playwright fallback. Module-level singleton browser+context+atexit (~1.5-2s saved per fetch × 50 fetches = 75-100s/run). Context-level route-block handler drops image/font/media/stylesheet/ad-hosts/paywall-scripts (40-60% per-page savings). JSON-LD articleBody ground truth via page.evaluate (zero hallucination on NYT/NewYorker/Atlantic/Reuters/BBC etc.). Trafilatura main-content extraction on hydrated DOM (already a repo dep). MutationObserver settle detection replaces hardcoded wait_for_timeout(1500). Multilingual autoconsent JS (single evaluate vs 7-selector × 1.5s loop). Hardened CrystallizeResult Pydantic schema (REPRODUCE VERBATIM mandate). gemma-2-9b dropped from OpenRouter chain (worst paraphrase offender). _score_extraction soft-failure detection. JS-disabled second context for static-render hosts (~200-500ms saved). LLM crystallizer DEFAULT OFF — opt-in via JEEVES_PW_USE_LLM_CRYSTALLIZE=1. This eliminates career sector's 14m55s OpenRouter retry storm (9 × 429s) since trafilatura/JSON-LD now handle most fetches without OpenRouter.
sprint-15-deps-2026-05-04: pyproject playwright extra now ships patchright>=1.40 + playwright>=1.40 (patchright preferred at import time). daily.yml: `playwright install --with-deps --only-shell chromium` (~30% smaller install, faster cold-start).
</state>
