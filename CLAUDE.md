...
**Phase 2/3 — quality sprint complete (2026-05-02).** Direct-to-main, no PR. Three independent failure layers driving repeated stories across the briefing, plus Playwright + OpenRouter as the universal article-fetch fallback.

Sprint 13 (quality) fixes:
- **Within-write-run topic tracking** (`write.py` ~3032): `used_topics_this_run` accumulates capitalised proper nouns, quoted titles, named acts/laws as each part is written. Subsequent parts get a "Topics already written" avoid-block injected into their system prompt — same pattern as `used_this_run` for asides, but for stories. `_extract_written_topics()` is regex-only.
- **DEDUP_PROMPT_HEADLINES_CAP raised 80 → 250** (`write.py:141`). Session has 205 same-day headlines; old cap meant Groq saw only 39% of prior coverage.
- **Cross-sector URL collision detection** (`research_sectors._find_cross_sector_dupes`, called from `scripts/research.py`): URLs appearing in 2+ sectors are stored on `session.dedup.cross_sector_dupes`. Schema field added to `Dedup` model.
- **Headline extraction quality** (`research_sectors._first_sentence` 150→250 chars; new `_first_two_sentences` 300 chars used for `findings`-like keys). Cross-day dedup matches on title+detail pair, not on a 150-char mid-phrase truncation.
- **NIM refine banned phrases** — 12 new filler phrases added to `_REFINE_SYSTEM` (positive-step / fascinating-contribution / will-be-monitoring / highlights-complexities cluster).
- **PART8 Library Stacks hardened** (`write.py:1019`): 7-character self-test (`<p></p>`), explicit FORBIDDEN OUTPUTS list (treasure trove, library's commitment, etc), no-pivot rule.
- **Playwright + OpenRouter universal article-fetch fallback** (`jeeves/tools/playwright_extractor.py`, NEW). Antifragile architecture: DOM sanitization (strip iframe/script/style/nav/footer/aside/svg + hidden attrs), markdown crystallization, Pydantic schema validation with parse-retry, circuit breaker (DOM-hash collision after 3 identical), action log (last 5 only), dead-end keyword detection (paywall/CAPTCHA/403/404 → `go_back`), action macros (`scroll_down`, `click_text`, `extract_main`, `done`, `give_up`), OpenRouter free-model fallback chain (llama-3.3-70b → gemma-2-9b → openrouter/auto). Two public APIs: `extract_article(url)` for the simple URL→text path, `run_navigation_session(start_url, objective)` for the full agent loop (future use). Wired as fallback into: `talk_of_the_town.fetch_full_article` (after Jina), `enrichment.fetch_article_text` (after httpx + trafilatura), `tavily.tavily_extract` (per-URL when Tavily returns `fetch_failed`). Also registered as a first-class agent tool `playwright_extract` in `jeeves/tools/__init__.py` so Kimi can invoke it directly. `research_system.md` prompt updated. Quota tracked under provider key `"playwright"`. Optional dep: `pip install -e ".[playwright]" && python -m playwright install chromium`. Soft-fails everywhere when playwright is absent — pipeline still ships.

**Test count: ≥249 passing locally** (was 187). 22 new sprint-13 quality tests + 41 new playwright_extractor tests.
