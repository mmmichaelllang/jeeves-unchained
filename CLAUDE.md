...
- **80 tests** in `tests/test_write_postprocess.py` cover the full write pipeline including refine/fallback behavior.

## Where we left off (2026-04-24, late)

- **PRs #16–#20 all merged to `main`.** The 9-call render (#16, #17, #18), the six quality fixes (#18), the TPD budget fix + NIM auto-fallback + concurrent NIM quality-editor pass (#20), plus the CLAUDE.md memory updates (#19).
- **Next step: re-run `write.yml` with `skip_send=true`** (once Groq TPD resets at midnight UTC, or immediately if relying on NIM fallback). Expected log pattern: 9 × `invoking Groq ... [partN]` lines with 65s gaps, interleaved with 9 × `NIM refine [partN]` lines. Final verification: (a) single greeting in Part 1; (b) ≥5 thematic profane asides with no apologies and no repeats; (c) no banned words/transitions; (d) no bare URLs; (e) Talk of the Town verbatim from `newyorker.text`; (f) choral/toddler/research items respect `dedup.covered_headlines`; (g) weather mentioned once, in Part 1 only.
