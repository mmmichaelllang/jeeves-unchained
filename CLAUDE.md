...
- **OpenRouter Llama 3.3 70B final narrative editor** (`_invoke_openrouter_narrative_edit`): runs ONCE on the full stitched+injected document. 14 editorial rules (A1–A14) cover filler, transitions, narrative cohesion, paragraph rhythm, weak openers, British wit amplification, vague attribution, and end-of-section summaries. Profanity placement (B1) handled here: exactly five asides, thematic match, no stacking. Skips NEWYORKER sentinels. Falls back silently. Requires `OPENROUTER_API_KEY`; model override via `OPENROUTER_MODEL_ID` (default `meta-llama/llama-3.3-70b-instruct`). `max_tokens=16384`, timeout=360s.
