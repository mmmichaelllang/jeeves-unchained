...
def test_timeout_counter_resets_on_successful_sector(
    ...
    """Counter resets to 0 after a successful sector.

    Sector 1 creates agents n=0 (initial, timeout) and n=1 (retry, timeout)
    → retry exhausted → counter=1. Threshold=2 so breaker NOT tripped.
    Sector 2 creates agent n=2 (success) → counter resets to 0.
    """
