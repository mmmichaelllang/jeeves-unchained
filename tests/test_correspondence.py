...
def test_classify_with_kimi_retries_on_429(monkeypatch):
    """A 429/rate-limit on the first attempt must trigger a sleep + retry,
    ...
    assert len(out) == 1
    assert out[0].id == "m0"
    # First retry sleep must come from the rate-limit schedule (>=60s).
...
def test_classify_with_kimi_inter_batch_sleep(monkeypatch):
    """Successful batches must be separated by a preemptive sleep so NIM
    doesn't 429 on later batches (this was the production failure mode that
    crashed the correspondence run on 2026-05-02)."""
    from llama_index.core.base.llms.types import ChatMessage
    ...
    assert len(out) == 45
    # Two inter-batch sleeps (between batches 1→2 and 2→3); none after
    # the final batch.
